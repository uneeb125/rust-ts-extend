...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
            ...
            ExprKind::CompartmentCast(e, idents) => {
                ...
                if self.tcx.compartments_enabled() && !compartments.is_empty() && !crate::cast::is_inside_compartment_unsafe_context(self.tcx, expr.hir_id) {
                    if let Some(mut err) = crate::compartments::compartment_diag(
                        ...
                        if let Some(src) = crate::cast::expanded_expr_note(self.tcx, expr) {
    ...
    fn check_expr_assign(
        ...
        if self.tcx.compartments_enabled() && !bypass && !rhs_compartments.tags.is_empty() && !current_compartments.can_access_with_trusted(&rhs_compartments, &trusted) {
            if let Some(mut err) = crate::compartments::compartment_diag(
                ...
                if let Some(src) = crate::cast::expanded_expr_note(self.tcx, rhs) {
                    err.note(format!("this code was generated by macro expansion:\n{src}"));
                }
                err.emit();
            }
            ...
            Ok(method) => {
                    ...
                    && !fn_compartments.can_access_with_trusted(&rcvr_compartments, &trusted)
                {
                    if let Some(mut err) = crate::compartments::compartment_diag(
                        ...
                        if let Some(src) = crate::cast::expanded_expr_note(self.tcx, rcvr) {
                            err.note(format!("this code was generated by macro expansion:\n{src}"));
                        }
                        err.emit();
                    }
                    ...
                    if let ty::Adt(adt_def, _) = arg_ty.kind() {
                        ...
                        if self.tcx.compartments_enabled() && !arg_bypass && !callee_is_trusted && !type_compartments.tags.is_empty() && !fn_compartments.can_access_with_trusted(&type_compartments, &trusted) {
                            if let Some(mut err) = crate::compartments::compartment_diag(
                                ...
                                if let Some(src) = crate::cast::expanded_expr_note(self.tcx, arg) {
                                    err.note(format!("this code was generated by macro expansion:\n{src}"));
                                }
                                err.emit();
                            }
    ...
    fn check_expr_cast(
        ...
        if let Err(guar) = (t_expr, t_cast).error_reported() {
            ...
            match cast::CastCheck::new(self, e, t_expr, t_cast, t.span, expr.span, Some(t)) {
                Ok(cast_check) => {
                    debug!(
