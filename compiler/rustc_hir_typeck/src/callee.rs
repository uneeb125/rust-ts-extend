    ...
    pub(crate) fn check_expr_call(
        ...
        let output = match result {
            ...
            Some(CallStep::DeferredClosure(def_id, fn_sig)) => {
                self.confirm_deferred_closure_call(call_expr, callee_expr, arg_exprs, expected, def_id, fn_sig)
            }
        ...
        if let Some(def_id) = def_id {
            ...
            if !skip_compartment_check {
                ...
                if self.tcx.compartments_enabled() && !bypass && !fn_compartments.tags.is_empty() && !current_compartments.can_access_with_trusted(&fn_compartments, &trusted) {
                    ...
                    if let Some(src) = crate::cast::expanded_expr_note(self.tcx, callee_expr) {
