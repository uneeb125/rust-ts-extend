...
use rustc_hir_pretty::expr_to_string;
...
///
/// Compartment diagnostics that fire on expressions emitted by a proc-macro
/// (e.g. `#[embassy_executor::main]`) are otherwise attributed to the macro
/// invocation site, which hides the actual offending expression. HIR is
/// post-expansion, so printing the node yields the targeted expanded form.
pub(super) fn expanded_expr_note(tcx: TyCtxt<'_>, expr: &hir::Expr<'_>) -> Option<String> {
    if expr.span.in_external_macro(tcx.sess.source_map()) {
        let src = expr_to_string(&tcx, expr);
        if !src.is_empty() {
            return Some(src);
        }
    }
        ...
        match e {
            ...
            CastError::CompartmentCastOutsideUnsafe => {
                if fcx.tcx.compartments_enabled() {
                    let mut err = fcx.dcx().struct_span_err(
                        self.span,
                        "compartment casts require an `unsafe` block",
                    );
                    if let Some(src) = expanded_expr_note(fcx.tcx, self.expr) {
                        err.note(format!("this code was generated by macro expansion:\n{src}"));
                    }
                    err.emit();
                }
            }
                ...
                if let ty::Uint(_) | ty::Int(_) = self.expr_ty.kind() {
                    ...
                    if let (Ok(src), Ok(dest)) = (src_layout, dest_layout) {
                        ...
                        if fcx.tcx.compartments_enabled() && src.size < dest.size {
                            let mut err = fcx.tcx.dcx().create_err(errors::UnsafePointerCastError {
                                span: self.expr_span,
                                note: "source integer is smaller than target pointee type"
                                    .to_string(),
                            });
                            if let Some(expanded) = expanded_expr_note(fcx.tcx, self.expr) {
                                err.note(format!(
                                    "this code was generated by macro expansion:\n{expanded}"
                                ));
                            }
                            err.emit();
                        }
